...
def load_all_vowels(str_classes, gender='both', sr=None, normalize=True, dir='data/vowels/', ext='.wav', max_samples=None, random_state=None):
    ...
    if max_samples is not None:
        ...
        x_m, _, y_m, _, F_m, _ = train_test_split(x_m, y_m, F_m, train_size=num_samples_men,   test_size=len(str_classes), stratify=y_m, shuffle=True, random_state=random_state)
        x_w, _, y_w, _, F_w, _ = train_test_split(x_w, y_w, F_w, train_size=num_samples_women, test_size=len(str_classes), stratify=y_w, shuffle=True, random_state=random_state)
